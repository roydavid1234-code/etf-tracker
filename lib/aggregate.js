...
// 權重變化：今/昨任一有值才算，否則 null（保持與 fixture 無 weight 時一致）
function _weightChange(todayW, prevW) {
  if (todayW == null && prevW == null) return null;
  return (todayW ?? 0) - (prevW ?? 0);
}

function _dailyDiffByPivot(snapshots, filterKey, filterValue, pivotKey, targetDate) {
  const byKey = new Map(); // pivotValue -> [{date, shares, weight}, ...]
  for (const s of snapshots) {
    ...
    byKey.get(s[pivotKey]).push({ date: s.snapshot_date, shares: s.shares, weight: s.weight ?? null });
  }
  const out = [];
  for (const [key, rows] of byKey) {
    rows.sort((a, b) => (a.date < b.date ? -1 : a.date > b.date ? 1 : 0));
    let today = 0, prev = 0, todayW = null, prevW = null;
    for (const r of rows) {
      if (r.date === targetDate) { today = r.shares; todayW = r.weight; }
      else if (r.date < targetDate) { prev = r.shares; prevW = r.weight; }
    }
    const change = today - prev;
    if (change !== 0) {
      out.push({ key, change, today, today_weight: todayW, weight_change: _weightChange(todayW, prevW) });
    }
  }
  return out;
}
...
function _rangeDiffByPivot(snapshots, filterKey, filterValue, pivotKey, startDate, endDate) {
  ...
  for (const [key, rows] of byKey) {
    rows.sort((a, b) => (a.date < b.date ? -1 : a.date > b.date ? 1 : 0));
    let baseline = 0, baselineW = null;
    for (const r of rows) {
      if (r.date < startDate) { baseline = r.shares; baselineW = r.weight; }
      else break;
    }
    const daily = [];
    let prev = baseline;
    let endShares = baseline, endWeight = baselineW;
    for (const r of rows) {
    ...
    if (total !== 0 || daily.length > 0) {
      out.push({ key, total, daily, end: endShares, end_weight: endWeight, weight_change: _weightChange(endWeight, baselineW) });
    }
  }
  return out;
}
...
function dailyChangeForStock(snapshots, stockCode, targetDate) {
  ...
  const result = raw.map(r => ({
    etf_code: r.key, change_shares: r.change, today_shares: r.today,
    today_weight: r.today_weight, weight_change: r.weight_change,
  }));
...
function rangeChangeForStock(snapshots, stockCode, startDate, endDate) {
  ...
  const result = raw.map(r => ({
    etf_code: r.key, total_change: r.total, daily: r.daily, end_shares: r.end,
    end_weight: r.end_weight, weight_change: r.weight_change,
  }));
...
function dailyChangeForEtf(snapshots, etfCode, targetDate) {
  ...
  const result = raw.map(r => ({
    stock_code: r.key, change_shares: r.change, today_shares: r.today,
    today_weight: r.today_weight, weight_change: r.weight_change,
  }));
...
function rangeChangeForEtf(snapshots, etfCode, startDate, endDate) {
  ...
  const result = raw.map(r => ({
    stock_code: r.key, total_change: r.total, daily: r.daily, end_shares: r.end,
    end_weight: r.end_weight, weight_change: r.weight_change,
  }));
